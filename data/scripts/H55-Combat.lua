...
-- function C55_IldarMana(side,faction,tier,qty1,qty2,qty3)	
	-- local creatures = GetUnits(side,CREATURE);
	-- for i,cr in creatures do
		-- if qty1 ~= 0 then
			-- if GetCreatureType(cr) == C55_Creatures[faction][tier][1] then
		...
		-- end;
		-- if qty3 ~= 0 then
			-- if GetGameVar("C55_Refined_Ildar") ~= "On" then
				-- if GetCreatureType(cr) == C55_Creatures[faction][tier][3] then
					-- local manacheck = GetUnitManaPoints(cr);
					-- local mana = GetUnitMaxManaPoints(cr)+qty3;
					-- if manacheck == 16 then
						-- SetUnitManaPoints(cr,mana);
					-- end;
				-- end;
			-- end;
...
function C55_ManaSpecialists(side,heroname)
	if heroname == "GodricMP" 	then C55_CastersMana(side,1,5,0,14,6) end;
	if heroname == "Ildar" 		then C55_CastersMana(side,2,4,6,14,0) end; --C55_IldarMana(side,2,4,6,14,8)   end;	
	if heroname == "Malustar" 	then C55_CastersMana(side,3,6,6,6,0)  end;
	if heroname == "Vidomina" 	then C55_CastersMana(side,4,5,0,6,14) end;
	if heroname == "Razzak" 	then C55_CastersMana(side,5,4,1,8,3)  end;
	if heroname == "ShadwynMP" 	then C55_CastersMana(side,6,6,7,14,6) end;
...
function C55_Combat(unitName)
	...
	-- if GetUnitSide(unitName) == ATTACKER and GetHero(ATTACKER) then
		-- local attacker_id = GetAttackerHero();
		-- local attacker = GetHeroName(attacker_id);
		-- if attacker == "Straker" then		
			-- if C55_TankMover[attacker] == 0 and unitName == attacker_id then
				-- C55_PullTank(ATTACKER,attacker,4,2);
				-- C55_TankMover[attacker] = 1;
			-- end;
			-- if C55_TankMover[attacker] == 1 and unitName ~= attacker_id then
				-- C55_TankMover[attacker] = 0;
			-- end;
		-- end;		
		-- if GetGameVar("C55_Ballista_"..attacker) == "On" or GetGameVar("C55_Brimstone_"..attacker) == "On" then		
			-- if C55_BallistaMover[attacker] == 0 and GetUnitType(unitName) == WAR_MACHINE and GetWarMachineType(unitName) == WAR_MACHINE_BALLISTA then
				-- C55_BallistaSalvo(ATTACKER,unitName);
				-- C55_BallistaMover[attacker] = 1;
			-- end;
		-- end;
		-- if C55_BallistaMover[attacker] == 1 then
			-- if GetUnitType(unitName) ~= WAR_MACHINE then
				-- C55_BallistaMover[attacker] = 0;
			-- elseif (GetUnitType(unitName) == WAR_MACHINE and GetWarMachineType(unitName) ~= WAR_MACHINE_BALLISTA) then
				-- C55_BallistaMover[attacker] = 0;
			-- end;
		-- end;
	-- end;
	-- if GetUnitSide(unitName) == DEFENDER and GetHero(DEFENDER) then
		-- local defender_id = GetDefenderHero(); 
		-- local defender = GetHeroName(defender_id);
		-- if defender == "Straker" then
			-- if C55_TankMover[defender] == 0 and unitName == defender_id then
				-- C55_PullTank(DEFENDER,defender,4,2);
				-- C55_TankMover[defender] = 1;
			-- end;
			-- if C55_TankMover[defender] == 1 and unitName ~= defender_id then
				-- C55_TankMover[defender] = 0;
			-- end;
		-- end;		
		-- if GetGameVar("C55_Ballista_"..defender) == "On" or GetGameVar("C55_Brimstone_"..defender) == "On" then		
			-- if C55_BallistaMover[defender] == 0 and GetUnitType(unitName) == WAR_MACHINE and GetWarMachineType(unitName) == WAR_MACHINE_BALLISTA then
				-- C55_BallistaSalvo(DEFENDER,unitName);
				-- C55_BallistaMover[defender] = 1;
			-- end;
		-- end;
		-- if C55_BallistaMover[defender] == 1 then
			-- if GetUnitType(unitName) ~= WAR_MACHINE then
				-- C55_BallistaMover[defender] = 0;
			-- elseif (GetUnitType(unitName) == WAR_MACHINE and GetWarMachineType(unitName) ~= WAR_MACHINE_BALLISTA) then
				-- C55_BallistaMover[defender] = 0;
