...
H55_LeadershipOwners = {}; H55_BaseLeadershipOwners = {};
...
H55_DemonicRetaliationOwners = {}; H55_DemonicRetaliationPlusOwners = {};
...
H55_TripleCatapultOwners = {}; H55_TripleCatapultExpertOwners = {}; H55_TripleCatapultRingOwners = {};
...
-- function H55_GetOccultismPowerLeveled(hero)
	...
	-- if HasHeroSkill(hero,SKILL_INVOCATION) then
		...
		-- end;	
	-- end;
	-- if (power >= 1) and HasHeroSkill(hero,SKILL_INVOCATION) and HasHeroSkill(hero,SKILL_DESTRUCTIVE_MAGIC) == nil then	
		-- power = power*2;
	-- end;
	-- return power;
-- end; 

function H55_GetOccultismPowerRebalanced(hero)
	local power = 0;
	local level = GetHeroLevel(hero);
...
function H55_InfoHeroManaRegen(hero)
	...
	--if HasHeroSkill(hero,WARLOCK_FEAT_PAYBACK) then mana=mana+4 end;	
	if HasHeroSkill(hero,RANGER_FEAT_INSIGHTS) then mana=mana+6 end;		
	...
	--if HasArtefact(hero,ARTIFACT_EIGHTFOLD,1) then	mana=mana+10 end;
	...
	--if HasArtefact(hero,ARTIFACT_UPG_ST2,1) then mana=mana+4 end;
	if HasArtefact(hero,ARTIFACT_RUNIC_WAR_HARNESS,1) then	mana=mana+5 end;
	--if HasArtefact(hero,ARTIFACT_BEGINNER_MAGIC_STICK,1) then mana=mana+2 end;
	if hero == "Astral" then
	...
	if blood >= 14 then
		...
		if (table.length(towns) > 0) then
			for i,town in towns do
				...
				if townrace == 8 then
					local growth = 3+(GetTownBuildingLevel(town, TOWN_BUILDING_SPECIAL_1))+(GetTownBuildingLevel(town, TOWN_BUILDING_SPECIAL_3));
					local totalgrowth = math.round((bloodroot/bloodcoef)*growth);
					elementals = elementals+totalgrowth;
				else
					local growth = 2+GetTownBuildingLevel(town, TOWN_BUILDING_MAGIC_GUILD);
					local totalgrowth = math.round((bloodroot/bloodcoef)*growth);
					elementals = elementals+totalgrowth;						
				end;
			end;
...
function H55_InfoLegion(hero)
	...
	--if HasArtefact(hero,ARTIFACT_CROWN_OF_LEADER,1) and HasHeroSkill(hero,PERK_RECRUITMENT) then bonus = bonus+5 end;
...
function H55_WeeklyEvents(player)	
	...
	if heroes ~= nil then
		...
		for i,hero in heroes do
			...
			--if hero == "Mephala" then H55_WeeklyRecruits("Mephala",player,CREATURE_TREANT,0.1,6) end;
			--if hero == "ThralsaiMP" then H55_WeeklyRecruits("ThralsaiMP",player,CREATURE_HYDRA,0.1,5) end;
			if hero == "ShadwynMP" then H55_WeeklyRecruits("ShadwynMP",player,CREATURE_MATRON,0.08,6) end;
			if hero == "GodricMP" then H55_WeeklyRecruits("GodricMP",player,CREATURE_PRIEST,0.13,5) end;
			--if hero == "Cyrus" then H55_WeeklyRecruits("Cyrus",player,CREATURE_GENIE,0.16,5) end;
			if hero == "Davius" then H55_WeeklyRecruits("Davius",player,CREATURE_RAKSHASA,0.08,6) end;
			--if hero == "Ash" then H55_WeeklyRecruits("Ash",player,CREATURE_NIGHTMARE,0.13,5) end;
			if hero == "Malustar" then H55_WeeklyRecruits("Malustar",player,CREATURE_PIT_FIEND,0.08,6) end;
			if hero == "Ufretin" then H55_WeeklyRecruits("Ufretin",player,CREATURE_THANE,0.08,6) end;
			if hero == "Vidomina" then H55_WeeklyRecruits("Vidomina",player,CREATURE_LICH,0.13,5) end;
			if hero == "Pelt" then H55_WeeklyRecruits("Pelt",player,CREATURE_WIGHT,0.08,6) end;
			if hero == "Nicolai" then H55_WeeklyRecruits("Nicolai",player,CREATURE_ANGEL,0.04,7) end;
			if hero == "Rissa" then H55_WeeklyRecruits("Rissa",player,CREATURE_GIANT,0.04,7) end;
			if hero == "Jenova" then H55_WeeklyRecruits("Jenova",player,CREATURE_GREEN_DRAGON,0.04,7) end;
			if hero == "Hangvul2" then H55_WeeklyRecruits("Hangvul2",player,CREATURE_FIRE_DRAGON,0.04,7) end;
			if hero == "Agbeth" then H55_WeeklyRecruits("Agbeth",player,CREATURE_DEEP_DRAGON,0.04,7) end;
			if hero == "Archilus" then H55_WeeklyRecruits("Archilus",player,CREATURE_BONE_DRAGON,0.06,7) end;
			if hero == "Harkenraz" then H55_WeeklyRecruits("Harkenraz",player,CREATURE_DEVIL,0.04,7) end;	
			if hero == "GottaiMP" then H55_WeeklyRecruits("GottaiMP",player,CREATURE_CYCLOP,0.04,7) end;
			
			if hero == "Mardigo" then H55_WeeklyReinforce("Mardigo",player,6,108,5,0.3) end; --0.25
			if hero == "Nathaniel" then H55_WeeklyReinforce("Nathaniel",player,2,106,1,0.9) end; --0.55
			if hero == "Straker" then H55_WeeklyReinforce("Straker",player,32,153,31,0.6) end;
			--if hero == "Isher" then H55_WeeklyReinforce("Isher",player,62,161,61,0.22) end;
			--if hero == "Darkstorm" then H55_WeeklyReinforce("Darkstorm",player,76,140,75,0.15) end;
			--if hero == "Nymus" then H55_WeeklyReinforce("Nymus",player,18,132,17,0.6) end;
			--if hero == "Matewa" then H55_WeeklyReinforce("Matewa",player,124,176,123,0.2) end;
			if hero == "Tamika" then H55_WeeklyReinforce("Tamika",player,36,155,35,0.2) end;	
			if hero == "Gem" then H55_WeeklyReinforce("Gem",player,44,145,43,0.8) end;
			if hero == "Ildar" then H55_WeeklyReinforce("Ildar",player,50,148,49,0.16) end;
			if hero == "Calh" then H55_WeeklyReinforce("Calh",player,16,131,15,0.9) end;
			if hero == "Nimbus" then H55_WeeklyReinforce("Nimbus",player,30,152,29,1.3) end;
			if hero == "Nemor" then H55_WeeklyReinforce("Nemor",player,34,154,33,0.45) end;
			if hero == "Gurvilin" then H55_WeeklyReinforce("Gurvilin",player,60,160,59,0.9) end;
			if hero == "Ferigl" then H55_WeeklyReinforce("Ferigl",player,78,141,77,0.15) end;
			if hero == "Maximus" then H55_WeeklyReinforce("Maximus",player,99,169,98,0.3) end;
			if hero == "Hero9" then H55_WeeklyReinforce("Hero9",player,118,173,117,1.6) end;
		end;
		...
		------------------------------------------------------------------------------------------------------------------------------------------------
			...
			if HasHeroSkill(hero,KNIGHT_FEAT_GUARDIAN_ANGEL) and (H55_GuardianAngelOwners[hero] ~= 1) then
...
function H55_DailyEvents(player)
	...
	for i,hero in heroes do
		...
		------------------------------------------------------------------------------------------------------------------------------------------------
		H55_DEBUG = {11,"AI WM",player,hero};--------------------------------------------------------------------------------------------------------
		------------------------------------------------------------------------------------------------------------------------------------------------	
		...
		if H55_IsThisAIPlayer(player) == 1 then
			...
			if GetHeroLevel(hero) >= 20 and HasHeroSkill(hero,PERK_BALLISTA) == nil and HasHeroWarMachine(hero,1) == nil then
				GiveHeroWarMachine(hero,1);
			end;
			if GetHeroLevel(hero) >= 20 and HasHeroSkill(hero,PERK_FIRST_AID) == nil and HasHeroWarMachine(hero,3) == nil then
				GiveHeroWarMachine(hero,3);
			end;				
		end;

		------------------------------------------------------------------------------------------------------------------------------------------------
		--H55_DEBUG = {11a,"Potions",player,hero};------------------------------------------------------------------------------------------------------
		------------------------------------------------------------------------------------------------------------------------------------------------
		...
		------------------------------------------------------------------------------------------------------------------------------------------------
		H55_DEBUG = {14,"SkillsDaily",player,hero};-----------------------------------------------------------------------------------------------------
		------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if heroes ~= nil then	
		...
		for i,hero in heroes do
			...
			-- if HasArtefact(hero,ARTIFACT_EIGHTFOLD,1) and GetHeroLevel(hero) >= 40 then H55_EightfoldStaffOwners[hero] ~= 1 then
				...
				-- if H55_IsThisAIPlayer(player) ~= 1 and H55_IsHeroInAnyTown(hero) == 0 then startThread(H55_ProtectedSign,"/Text/Game/Scripts/Spellpower.txt",hero,player,H55_MsgSleep,H55_MsgTime) end;
			-- end;
			...
			if H55_RPGPotions == 1 then
				...
				if HasArtefact(hero,ARTIFACT_POTION03,1) and (H55_ResurrectPotionOwners[hero] ~= 1) then
					H55_ResurrectPotionOwners[hero] = 1;
					if H55_IsThisAIPlayer(player) ~= 1 and H55_IsHeroInAnyTown(hero) == 0 then 
						startThread(H55_ProtectedSign,"/Text/Game/Scripts/RespotionEq.txt",hero,player,H55_MsgSleep,H55_MsgTime);
					end;
				end;
			...
			if HasHeroSkill(hero,PERK_MASTER_OF_CURSES) and KnowHeroSpell(hero,H55_SpellsL1[2]) == nil then
				TeachHeroSpell(hero,H55_SpellsL1[2]);
				if H55_IsThisAIPlayer(player) ~= 1 and H55_IsHeroInAnyTown(hero) == 0 then startThread(H55_ProtectedSignSpell,"/Text/Game/Scripts/Learnspell.txt",H55_SpellsTextL1[2],hero,player,H55_MsgSleep,H55_MsgTime) end;				
			end;			
			...
			if HasHeroSkill(hero,DEMON_FEAT_FIRE_AFFINITY) and KnowHeroSpell(hero,H55_SpellsL1[10]) == nil then
				TeachHeroSpell(hero,H55_SpellsL1[10]);
				if H55_IsThisAIPlayer(player) ~= 1 and H55_IsHeroInAnyTown(hero) == 0 then startThread(H55_ProtectedSignSpell,"/Text/Game/Scripts/Learnspell.txt",H55_SpellsTextL1[10],hero,player,H55_MsgSleep,H55_MsgTime) end;				
			end;	
			if HasHeroSkill(hero,NECROMANCER_FEAT_DEAD_LUCK) and KnowHeroSpell(hero,H55_SpellsL3[3]) == nil then
			...
			if HasHeroSkill(hero,DEMON_FEAT_MASTER_OF_SECRETS) and KnowHeroSpell(hero,H55_SpellsL3[1]) == nil then
			...
			if HasHeroSkill(hero,NECROMANCER_FEAT_DEADLY_COLD) and KnowHeroSpell(hero,H55_SpellsL3[5]) == nil then
				TeachHeroSpell(hero,H55_SpellsL3[5]);
				if H55_IsThisAIPlayer(player) ~= 1 and H55_IsHeroInAnyTown(hero) == 0 then startThread(H55_ProtectedSignSpell,"/Text/Game/Scripts/Learnspell.txt",H55_SpellsTextL3[5],hero,player,H55_MsgSleep,H55_MsgTime) end;				
			end;
			if HasHeroSkill(hero,DEMON_FEAT_WEAKENING_STRIKE) and KnowHeroSpell(hero,H55_SpellsL2[1]) == nil and table.contains(H55_Chieftains,hero) then
			...
			if HasHeroSkill(hero,PERK_PRAYER) and (H55_GetHeroRaceNum(hero) == 4) then
			...
			-- if H55_IsHeroInAnyTown(hero) == 1 then
				-- if HasArtefact(hero,ARTIFACT_SENTINEL,1)  and H55_SentinelOwners[hero] ~= 1 then
			...
			if HasHeroSkill(hero,NECROMANCER_FEAT_TWILIGHT) and (GetHeroLevel(hero) >= 30) and (H55_TwilightOwners[hero] ~= 1) then
			...
			if HasHeroSkill(hero,KNIGHT_FEAT_PARIAH) and (GetHeroLevel(hero) >= 30) and (H55_PariahOwners[hero] ~= 1) then
			...
			-- if HasHeroSkill(hero,NECROMANCER_FEAT_CHILLING_BONES) == nil and (H55_ChillingBonesOwners[hero] == 1) then
				...
				-- ChangeHeroStat(hero,STAT_DEFENCE,-2);
			-- end;
			-- if HasHeroSkill(hero,NECROMANCER_FEAT_CHILLING_BONES) and (H55_GetHeroRaceNum(hero) ~= 4) and (H55_ChillingBonesOwnersLife[hero] ~= 1) then
			...
			if HasHeroSkill(hero,HERO_SKILL_BARBARIAN_DARK_REVELATION) and (H55_DarkRevelationOwners[hero] ~= 1) then
			...
			if HasHeroSkill(hero,HERO_SKILL_BARBARIAN_ELITE_CASTERS) and (H55_BRefinedManaOwners[hero] ~= 1) then
			...
			if (H55_IsThisAIPlayer(player) ~= 1) and HasHeroSkill(hero,HERO_SKILL_VOICE) and HasHeroSkill(hero,HERO_SKILL_EMPATHY) and (H55_EmpathyOwners[hero] ~= 1) then
			...
			if HasHeroSkill(hero,DEMON_FEAT_MASTER_OF_SECRETS) and (GetHeroLevel(hero) >= 20) and (H55_ArcaneExaltationOwners[hero] ~= 1) then
			...
			if HasHeroSkill(hero,WARLOCK_FEAT_SECRETS_OF_DESTRUCTION) and (GetHeroLevel(hero) >= 20) and (H55_SecretsDestructionOwners[hero] ~= 1) then
			...
			if HasHeroSkill(hero,DEMON_FEAT_TRIPLE_CATAPULT) and HasHeroWarMachine(hero,4) and (H55_TripleCatapultOwners[hero] ~= 1) then
			...
			if HasHeroSkill(hero,DEMON_FEAT_TRIPLE_CATAPULT) and GetHeroSkillMastery(hero,SKILL_WAR_MACHINES) == 3 and HasHeroWarMachine(hero,4) and (H55_TripleCatapultExpertOwners[hero] ~= 1) then
			...
			if HasHeroSkill(hero,DEMON_FEAT_TRIPLE_CATAPULT) and HasHeroWarMachine(hero,4) and HasArtefact(hero,ARTIFACT_RING_OF_MACHINE_AFFINITY,1) and (H55_TripleCatapultRingOwners[hero] ~= 1) then
			...
			if H55_NecropolisPhonebook[hero] == 1 then		
				if HasHeroSkill(hero,SKILL_LEADERSHIP) then
					...
					if (GetHeroSkillMastery(hero,SKILL_LEADERSHIP) == 1) and ((H55_LeadershipOwners[hero] == nil) or (H55_LeadershipOwners[hero] == 0)) then
						...
						ChangeHeroStat(hero,STAT_ATTACK,4);
						if H55_IsThisAIPlayer(player) ~= 1 and H55_Day ~= 1 and H55_IsHeroInAnyTown(hero) == 0 then startThread(H55_ProtectedSign,"/Text/Game/Scripts/Attack.txt",hero,player,H55_MsgSleep,H55_MsgTime) end;
					end;
			...
			if H55_AfterBattleMana[hero] >= 1 then
				H55_ModifyMana(hero,H55_AfterBattleMana[hero]);
			end;
			...
			if HasArtefact(hero,ARTIFACT_MEDICAL02,1) and (H55_CIResurrectT7[hero] ~= ci) and (GetSavedCombatArmyHero(ci,1)==hero) and (H55_GetSpecialNecroReq(hero) == 0) and (H55_GetHeroRaceNum(hero) ~= 7) and (H55_RefuseCIResurrectT7[hero] ~= ci) then
				H55_ResurrectArmy(player,hero,ci,7,18,0.3);
				H55_CIResurrectT6[hero] = ci;
			end;
			...
			if HasArtefact(hero,ARTIFACT_GEAR_02,1) and H55_RefuseCIResurrectT6[hero] == ci and (GetSavedCombatArmyHero(ci,1)==hero) and H55_RefuseAgainCIResurrectT6[hero] ~= ci and (H55_CIResurrectT6[hero] ~= ci) and (H55_GetSpecialNecroReq(hero) == 0) then
			...
			if HasArtefact(hero,ARTIFACT_GEAR_02,1) and H55_RefuseCIResurrectT5[hero] == ci and (GetSavedCombatArmyHero(ci,1)==hero) and H55_RefuseAgainCIResurrectT5[hero] ~= ci and (H55_CIResurrectT5[hero] ~= ci) and (H55_GetSpecialNecroReq(hero) == 0) then
			...
			if HasArtefact(hero,ARTIFACT_MEDICAL04,1) and H55_RefuseCIResurrectT4[hero] == ci and (GetSavedCombatArmyHero(ci,1)==hero) and H55_RefuseAgainCIResurrectT4[hero] ~= ci and (H55_CIResurrectT4[hero] ~= ci) and (H55_GetSpecialNecroReq(hero) == 0) then
			...
			if HasArtefact(hero,ARTIFACT_MEDICAL04,1) and H55_RefuseCIResurrectT3[hero] == ci and (GetSavedCombatArmyHero(ci,1)==hero) and H55_RefuseAgainCIResurrectT3[hero] ~= ci and (H55_CIResurrectT3[hero] ~= ci) and (H55_GetSpecialNecroReq(hero) == 0) and (H55_GetHeroRaceNum(hero) ~= 5) then
			...
			if HasArtefact(hero,ARTIFACT_MEDICAL03,1) and H55_RefuseCIResurrectT2[hero] == ci and (GetSavedCombatArmyHero(ci,1)==hero) and H55_RefuseAgainCIResurrectT2[hero] ~= ci and (H55_CIResurrectT2[hero] ~= ci) and (H55_GetSpecialNecroReq(hero) == 0) and (H55_GetHeroRaceNum(hero) ~= 5) then
			...
			if HasArtefact(hero,ARTIFACT_MEDICAL03,1) and H55_RefuseCIResurrectT1[hero] == ci and (GetSavedCombatArmyHero(ci,1)==hero) and H55_RefuseAgainCIResurrectT1[hero] ~= ci and (H55_CIResurrectT1[hero] ~= ci) and (H55_GetSpecialNecroReq(hero) == 0) then
			...
			if HasArtefact(hero,ARTIFACT_ORB_01,1) and (H55_CIResurrectAir[hero] ~= ci) and (GetSavedCombatArmyHero(ci,1)==hero) and (H55_RefuseCIResurrectAir[hero] ~= ci) then
				H55_ResurrectElemental(player,hero,ci,88,10,0.3);
			...
			if HasArtefact(hero,ARTIFACT_ORB_02,1) and (H55_CIResurrectEarth[hero] ~= ci) and (GetSavedCombatArmyHero(ci,1)==hero) and (H55_RefuseCIResurrectEarth[hero] ~= ci) then
				H55_ResurrectElemental(player,hero,ci,87,10,0.3);
			...
			if HasArtefact(hero,ARTIFACT_ORB_03,1) and (H55_CIResurrectFire[hero] ~= ci) and (GetSavedCombatArmyHero(ci,1)==hero) and (H55_RefuseCIResurrectFire[hero] ~= ci) then
				H55_ResurrectElemental(player,hero,ci,85,10,0.3);
			...
			if HasArtefact(hero,ARTIFACT_ORB_04,1) and (H55_CIResurrectWater[hero] ~= ci) and (GetSavedCombatArmyHero(ci,1)==hero) and (H55_RefuseCIResurrectWater[hero] ~= ci) then
				H55_ResurrectElemental(player,hero,ci,86,10,0.3);
...
do
	...
	if H55_GenerateMixedStacks == 1 and H55_MapType == "Multiplayer" and H55_ForceMixedStacks == 0 and H55_MixedByMapmixer == 0 then
		if H55_AskForMixedStacks == 1 then
	...
	elseif H55_ForceMixedStacks == 1 then --or table.contains(H55_MultiplayerMaps,GetMapDataPath())
		H55_SkipModifyStackSize = 1;
		startThread(H55_MixStacks);
	else
		H55_MixingStacksComplete = 1;
